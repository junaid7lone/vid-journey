...
client
  ...
  .setProject(awConfig.projectId)
  .setPlatform(awConfig.platform);
...
export async function getFilePreview(fileId, type) {
  ...
  try {
    if (type === "video") {
      fileUrl = storage.getFileView(awConfig.storageId, fileId);
    } else if (type === "image") {
...
export async function bookmarkPost(form) {
  const { userId, video } = form;

  const { bookmarkedBy } = video;

  let newBookmarkValue;

  if (bookmarkedBy.includes(userId)) {
    newBookmarkValue = bookmarkedBy.filter((user) => user !== userId);
  } else {
    newBookmarkValue = [...bookmarkedBy, userId];
  }

  try {
    const newBookmark = await databases.updateDocument(
      awConfig.databaseId,
      video.$collectionId,
      video.$id,
      {
        bookmarkedBy: newBookmarkValue,
      }
    );
...
export async function getUserBookmarks(userId) {
  try {
    const posts = await databases.listDocuments(
      awConfig.databaseId,
      awConfig.videoCollectionId,
      [Query.orderDesc("$createdAt"), Query.contains("bookmarkedBy", userId)]
    );
